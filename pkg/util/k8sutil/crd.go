...
// ReplicatedStatefulSetCRUpdateFunc is a function to be used when atomically
...
func GetClusterList(restcli rest.Interface, ns string) (*api.ReplicatedStatefulSetList, error) {
	...
	clusters := &api.ReplicatedStatefulSetList{}
...
func listClustersURI(ns string) string {
	return fmt.Sprintf("/apis/%s/namespaces/%s/%s", api.SchemeGroupVersion.String(), ns, api.ReplicatedStatefulSetResourcePlural)
}
