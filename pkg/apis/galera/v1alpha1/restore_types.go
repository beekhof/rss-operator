...
// GaleraRestoreList is a list of GaleraRestore.
...
// GaleraRestore represents a Kubernetes GaleraRestore Custom Resource.
// The GaleraRestore CR name will be used as the name of the new restored cluster.
...
// RestoreSpec defines how to restore an galera cluster from existing backup.
type RestoreSpec struct {
	// ClusterSpec defines the same spec that galera operator will run later.
	// using this spec, restore operator will prepare the seed that
	// galera operator will pick up later.
...
type S3RestoreSource struct {
	...
	// e.g: "galera-backups/v1/default/example-galera-cluster/3.2.10_0000000000000001_galera.backup"
	...
	//
	// AWSSecret overwrites the default galera operator wide AWS credential and config.
