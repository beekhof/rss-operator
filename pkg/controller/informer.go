...
func (c *Controller) Start() error {
	...
	c.logger.Infof("resources initialized")
	probe.SetReady()
	c.run()
	panic("unreachable")
}
...
func (c *Controller) run() {
	source := cache.NewListWatchFromClient(
		c.Config.EtcdCRCli.ClusterlabsV1alpha1().RESTClient(),
		api.ReplicatedStatefulSetResourcePlural,
		c.Config.Namespace,
		fields.Everything())

	_, informer := cache.NewIndexerInformer(source, &api.ReplicatedStatefulSet{}, 0, cache.ResourceEventHandlerFuncs{
...
func (c *Controller) onUpdateEtcdClus(oldObj, newObj interface{}) {
	c.syncEtcdClus(newObj.(*api.ReplicatedStatefulSet))
}

func (c *Controller) onDeleteEtcdClus(obj interface{}) {
	clus, ok := obj.(*api.ReplicatedStatefulSet)
	if !ok {
		tombstone, ok := obj.(cache.DeletedFinalStateUnknown)
		if !ok {
			panic(fmt.Sprintf("unknown object from ReplicatedStatefulSet delete event: %#v", obj))
		}
		clus, ok = tombstone.Obj.(*api.ReplicatedStatefulSet)
		if !ok {
			panic(fmt.Sprintf("Tombstone contained object that is not an ReplicatedStatefulSet: %#v", obj))
		}
	}
	ev := &Event{
		Type:   kwatch.Deleted,
...
func (c *Controller) syncEtcdClus(clus *api.ReplicatedStatefulSet) {
	ev := &Event{
		Type:   kwatch.Added,
		Object: clus,
	}
